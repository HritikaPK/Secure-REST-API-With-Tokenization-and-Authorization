...
app.get("/fetch-loan-status", auth, async (req, res) => {        //get: get request; "/" : req sent to root ;
    // you can also send msg in html

    try {
        const cli = await UserLogs.findOne({ Username: req.body.Username, Password: req.body.Password })
        ...
        if (cli.Role === "Admin" || cli.Role === "Client") {
            ...
            return res.status(200).send("The following is the loan status of the client:"+rec.loanstat);
...
app.post("/create-new-client", async (req, res) => {        //post: post request; "/" : req sent to root ;
    try{
        const cli = await UserLogs.findOne({ Username: req?.body?.UsernameAdmin, Password: req?.body?.PasswordAdmin })

        if (req.body.refferalCode === "AdminPass"|| (cli && cli.Role==="Admin")){
        ...
        await jwt.sign({ id: newUser.id }, process.env.JWT_SECRET, async (err, token) => {
            if (err)
                ...
                const UserLogss = await UserLogs.create({ idnum: newUser._id, Username: req.body.Username, Password: req.body.Password, Role: req.body.Role, hashedPassword });
...
app.put("/update-user-info", auth, async (req, res) => {        //put: put request; "/" : req sent to root ;
    ...
    try {
        if (cli.Role === "Admin"||req.body.refferalCode === "AdminPass") {
            ...
            const upd = await Client.updateOne({ _id: req.body.idnum }, { $set: { firstname: req.body.firstname, lastname: req.body.lastname, phnum: req.body.phnum } })
            return res.status(200).send(upd);
        }
        else
        return res.status(404).json({message:"No Cli found"})
    }
    catch (err) {
...
app.put("/update-user-log", auth, async (req, res) => {        //put: put request; "/" : req sent to root ;


    //login
    const cli = await UserLogs.findOne({ Username: req.body.Username, Password: req.body.Password })
    try {
        if (cli.Role === "Admin"||req.body.refferalCode === "AdminPass") {  //if person is admin

            //send entire data with updated data as well
            const upd = await UserLogs.updateOne({ idnum: req.body.idnum }, { $set: { Username: req.body.Username, Password: req.body.Password, Role: req.body.Role } })
...
app.delete("/delete-client-data", auth, async (req, res) => {        //Delete: delete request; "/" : req sent to root ;
...
app.delete("/delete-client-record", auth, async (req, res) => {        //Delete: delete request; "/" : req sent to root ;
